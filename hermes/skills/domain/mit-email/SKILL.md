...
- Stronger failure mode discovered on this Mac mini: native Apple Mail `reply ...` messages can save or send with an effectively empty body after AppleScript body rewriting, even without attachments. One observed trigger is rewriting reply content with a pattern like `set content to (customText & return & return & content)`; failures may leave only blank newlines or an attachment marker. Treat native reply-body rewriting as unreliable here.
